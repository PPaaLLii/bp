\documentclass[a4paper, upjsfrontpage, disablespecwarning, thesismargins, thesislinespacing]{rnthesis}
...
\abstrakt{
\noindent Hlavným cieľom našej bakalárskej práce je navrhnúť spôsob monitorovania informačného systému za účelom zbierania užitočných informácií ohľadom výkonu systému a používateľských preferencií. Zaoberáme sa najmä otázkami, aké informácie zbierať a akými nástrojmi ich zbierať. Existuje viacero nástrojov sčasti riešiacich našu problematiku. Zameriame sa na open-source nástroje. V\-práci si rozoberieme tie najpoužívanejšie, ich výhody, nevýhody a vybrané nástroje integrujeme do akademického informačného systému AiS2. Pomocou nich sa pokúsime zbierať a zobrazovať metriky, ktoré nám pomôžu identifikovať problémy a funkcie, ktoré je potrebné vylepšiť.
~\\
~\\
{\bf Kľúčové slová:} metrika, monitorovanie, informačný systém}

\abstract{
\noindent The main aim of our bachelor thesis is to design a way to monitor information system in order to gather useful information about system performance and user preferences.
 We are dealing mainly with questions what information to gather and what tools to use for collecting.
  There are several available open-source tools to solve certain parts of our problems. 
  In this thesis we will be describing advantages and disadvantages of the best known of the tools and integrating choosen ones into the academical information system AiS2. 
  We will gather and vizualize useful metrics using the tools to identify problems and functions to be improved.
  ~\\
~\\
{\bf Key words:} metric, monitoring, information system}
...
\cite{1}
%http://en.wikipedia.org/wiki/Information\_system
...
\cite{2}
%{website:wikipedia-webaplikacia}
...
V nasledujúcich odsekoch si popíšeme ich výhody a nevýhody.
...
Má klientov pre množstvo programovacích jazykov ako napríklad node.js, java, python, ruby, perl, php, c, cpp, .net, či dokonca plugin pre wordpress. 
...
Obe nástroje slúžia na zbieranie metrík, majú však rozdielnu architektúru. 
Ponúkajú rôzne možnosti integrácie s ďalšími nástrojmi. 
Kvôli prehľadnosti sú základné rozdiely popísané v tabuľke 5.1.
...
Na obrázku 6.2 môžeme vidieť používateľské rozhranie nástroja Graphite a čiarový diagram pre metriku zaťaženie procesora.
...
Nástroj Graphite je zameraný len na vykresľovanie metrík a neponúka žiadnu dodatočnú funkcionalitu.
...
Jeho kód je napísaný v jazyku C a~používateľské rozhranie v podobe webovej aplikácie je napísané v jazyku PHP.
...
Ďalším obľúbeným nástrojom je Ganglia.
...
Webové používateľské rozhranie potom zobrazí základný graf pre každú metriku.
...
Gmond medzi sebou komunikujú buď prostredníctvom unicastu alebo multicastu.
...
Na obrázku 6.6 môžeme vidieť používateľské rozhranie nástroja Ganglia.
...
Ich integrácia je ale cez Gmetric, čo~ale obchádza infraštruktúru s použitím Gmond modulov, čo je jedným zo základných benefitov návrhu nástroja Ganglia.
...
Pre testovacie účely sme si vytvorili jednoduchú webovú aplikáciu.
...
Metrics sme si vybrali aj preto, lebo komunikácia medzi Statsd klientom a Statsd serverom po sieti môže spôsobiť zbytočné zaťaženie siete.
...
Gangliu sme si vybrali hlavne preto, že ponúka riešenie pre agregáciu metrík z viacerých inštalácií systému AiS2 na jedno miesto.
...
Gangliu sa nám však nepodarilo nasadiť na najnovšiu verziu Red Hat distribúcie, pretože Ganglia v čase písania tejto práce ešte nebola pre túto verziu dostupná.
...
Každú metriku však v Zabbixe bolo potrebné najprv nakonfigurovať.

V konfigurácii metriky bolo treba nastaviť jej meno, 
...
Ak zaznamenajú novú metriku, jednoducho v strome metrík si~vytvoria nový adresár podľa názvu a prefixu metriky.
...
Ak by sa~napríklad stalo, že používateľ by otvoril dialóg, o chvíľu by ho zavrel, pootváral by~ďalšie, možno podobné dialógy, a nakoniec by sa vrátil k prvému, vyplnil by ho a odoslal, je pravdepodobné že správny dialóg hľadal, ale nenašiel ho hneď.
Ak sa tento scenár opakuje často u rôznych používateľov systému, pravdepodobne nie je dialóg, či ľubovoľná iná informácia v systéme vhodne navrhnutá alebo je v systéme zle umiestnená.
...
Preto má pre nás detekcia takýchto dialógov zmysel.
...
Neskôr sme si ale~uvedomili, že síce Zabbix na ukladanie metrík používa relačnú databázu, ale takúto komplexnú metriku mu nevieme poslať zo žiadneho nástroja na zbieranie metrík, ktoré sme vyššie opisovali, keďže podporujú iba zbieranie jednoduchých metrík.
...
Mali by sme tak užitočné informácie, ktoré by nám pomohli zlepšiť interakciu systému s používateľom informačného systému.
...
%\bibitem{1} Ipsum, L.: Lorem ipsum dolor sit amet.
	...
	\bibitem{1} Information system. Wikipedia: the free encyclopedia [online]. San Francisco (CA): Wikimedia Foundation, 2001- [cit. 2015-05-17]. Dostupné z: http://en.wikipedia.org/wiki/Information\_system
	\bibitem{2}  Webová aplikácia. Wikipedia: the free encyclopedia [online]. San Francisco (CA): Wikimedia Foundation, 2001- [cit. 2015-05-17]. Dostupné z: http://sk.wikipedia.org/wiki/Webov\%C3\%A1\_aplik\%C3\%A1cia
	\bibitem{3} SMOLEN, Eduard. Metriky [online]. [cit. 2015-05-17]. Dostupné z: http://epodnikanie.euin.org/node/126
	\bibitem{4} KAUSHIK, Avinash. Webová analytika 2.0: kompletní průvodce analýzami návštěvnosti. Vyd. 1. Brno: Computer Press, 2011. ISBN 978-80-251-2964-7.
	\bibitem{5} MATT MASSIE, Bernard Li. Monitoring with Ganglia. 1. ed. Sebastopol, CA: O'Reilly, 2013. ISBN 9781449329709.
	\bibitem{6} ALLSPAW, John a Jesse ROBBINS. Web operations. Sebastopol, CA: O'Reilly, 2010, s. 43-44. ISBN 1449377440.
\end{thebibliography}
%
\newpage
...
Príloha B: \tab{JSP stránka s integrovaným Statsd klientom} \\
Príloha C: \tab{JSP stránka s integrovanou knižnicou Metrics} \\


% TODO zoznam priloh + maju byt v obsahu?
